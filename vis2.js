...
var m4Scale = d3.scaleLinear()
            .range([10, 0])
            .domain([0, 125000])

var x4Axis = d3.axisBottom(x4Scale);
var y4Axis = d3.axisLeft(y4Scale);

Svg.append("g")
            .call(y4Axis)
        var x4Axis = Svg.append("g")
  ...
  scatter
    ...
    .enter()
    .append("circle")
      .attr("cx", function (d) { return x4Scale(d.manufacture_year); } )
      .attr("cy", function (d) { return y4Scale(d.price) - 10; } )
      .attr("r", function (d) { return m4Scale(d.mileage); } )
      .style("fill", function (d) { return color(d.maker) } )
      .style("opacity", 0.5)
      .on("mouseover", function (d, i) {
                bar_tooltip.transition()
  ...
  function updateChart() {
    ...
    if(!extent){
      ...
      x4Scale.domain([1960,2020])
    }else{
      x4Scale.domain([ x4Scale.invert(extent[0]), x4Scale.invert(extent[1]) ])
      scatter.select(".brush").call(brush.move, null) // This remove the grey brush area as soon as the selection has been done
    }

    // Update axis and circle position
    x4Axis.transition().duration(1000).call(d3.axisBottom(x4Scale))
    scatter
      .selectAll("circle")
